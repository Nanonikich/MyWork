...
class MainWindowClass(QMainWindow, formMain):
	...
	def showWorkWindow(self):
		self.wsi = WorkerSignIn() #worker sign in
		self.wsi.show()
		self.close()
	...
	def showAdminWindow(self):
		self.asi = AdminSignIn() #admin sign in
		self.asi.show()
		self.close()
...
class WorkerSignIn(QMainWindow, formWorkSignIn):
	...
	def showMain(self):
		self.mw = MainWindowClass() #main window
		self.mw.show()
		self.close()
	...
	def show_RegWindow(self):
		self.wr = RegWorkWindow() #worker reg
	...
	def check(self):
		...
		if data != False:
			self.close()
			self.ws = worker_space.WorkerClass(data) #worker space
			self.ws.show()
		else:
			self.ew = ErrorClass() #error window
			self.ew.show()
...
class AdminSignIn(QMainWindow, formAdminSignIn):
	...
	def show_RegWindow(self):
		self.rw = RegAdminWindow() #reg window
	...
	def check(self):
		...
		if data != False:
			self.close()
			self.adms = admin_space.AdminTableClass(data) # admin space
			self.adms.show()
		else:
			self.ew = ErrorClass() #error window
			self.ew.show()
...
class RegWorkWindow(QDialog, formWorkReg):
	...
	def showMessBox(self):
		self.mb = MessBoxClass() #message box
		self.mb.show()
